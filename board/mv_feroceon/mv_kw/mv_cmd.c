...
#if defined(CONFIG_BUFFALO_PLATFORM)
	extern void bfKeepEthAddr(void);
	bfKeepEthAddr();
#endif
...
#if defined(CONFIG_BUFFALO_PLATFORM)
int bfTestWol(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	MV_U8 mac[6];
	char env_ethaddr[16];
	char *env;
	...
	int target_eth = 0;
	int verbose = 0;
	int gmagic_wait = 0;	/* generic magic wait flag. using bfSetWolMagicWaitMode*/
	int flag_ope_disable = 0;

	for(i = 1; i < argc; i++)
	{
		if((strlen(argv[i]) == 1) && (argv[i][0] >= '0' && argv[i][0] <= '9'))
		{
			target_eth = simple_strtoul(argv[i], &end, 10);
		}
		else if(strncmp(argv[i], "v", 1) == 0)
		{
			verbose = 1;
		}
		else if(strncmp(argv[i], "m", 1) == 0)
		{
			gmagic_wait = 1;
		}
		else if(strncmp(argv[i], "d", 1) == 0)
		{
			flag_ope_disable = 1;
		}
	}

	memset(env_ethaddr, 0, sizeof(env_ethaddr));
	if(target_eth == 0)
	{
		sprintf(env_ethaddr, "ethaddr");
	}
	else
	{
		if(target_eth >= 2)
			return -1;
		sprintf(env_ethaddr, "eth%daddr", target_eth);
	}

	printf("target is eth%d\n", target_eth);

	if(flag_ope_disable == 1)
	{
		bfControlWolInterrupt(target_eth, FLAG_BUFFALO_WOL_INTERRUPT_DISABLE);
	}
	else
	{
		if(gmagic_wait == 1)
			bfSetWolMagicWaitMode(target_eth);
		else
			bfSetWolFrameWaitMode(target_eth, verbose);
	}
}

U_BOOT_CMD(
        test_wol, 4, 0, bfTestWol,
	"test_wol- wol test suite.\n", 
	"test_wol\n"
	"test_wol v - test WOL mode by verbose mode. default is non verbose mode.\n"
	"test_wol m - test magic patern mode(not a frame matching). default is frame patern matching.\n"
	"test_wol d - disable WOL mode.\n"
	"test_wol 1 - test WOL by port 1. default is 0.\n"
);

int bfGetWolStatus(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	MV_U8 i = 0;
	int target_eth = 0;
	int loop = 0;
	int flag_reset = 0;
	MV_U8	disp_interval = 100;
	char *end;
	MV_U16 wol_stat = 0;
	...
	if(loop == 1)
	{
		while (1)
		{
			wol_stat = bfGetWolInterruptStatus(target_eth, 0);
			if(wol_stat)
			{
				printf("I've received WOL packet!(0x%04x : %s)\n", wol_stat, bfGetWolPatternString(wol_stat));
				break;
			}
			if(disp_interval > 0)
				disp_interval--;
			...
			{
				disp_interval = 100;
				printf("I'm waiting WOL packet...(0x%04x)\n", wol_stat);
			}
			mvOsDelay(100);
		}
	}
	...
	{
		bfGetWolInterruptStatus(target_eth, 1);
	}
}

U_BOOT_CMD(
        chk_wol, 3, 0, bfGetWolStatus,
	"chk_wol - wol test suite.\n", 
	"chk_wol\n"
	"chk_wol l - check wol status as a daemon mode.\n"
	"chk_wol r - reset cpu.\n"
);
#endif /* CONFIG_BUFFALO_PLATFORM) */
